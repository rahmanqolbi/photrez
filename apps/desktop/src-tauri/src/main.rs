...
const NATIVE_MENU_EVENT: &str = "photrez://native-menu";
const EDITOR_MENU_IDS: &[&str] = &[
    "file.new",
    "file.open",
    "file.export",
    "edit.undo",
...
fn build_native_menu<R: Runtime, M: Manager<R>>(manager: &M) -> tauri::Result<Menu<R>> {
...
fn is_editor_menu_id(id: &str) -> bool {
...
fn main() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            app.set_menu(build_native_menu(app)?)?;
            app.on_menu_event(|app_handle, event| {
                let id = event.id().0.as_str();
                if is_editor_menu_id(id) {
                    let _ = app_handle.emit(NATIVE_MENU_EVENT, id);
                }
            });
