...
| Architecture (future target) | Hybrid modular: `Tauri shell + Rust core (photrez-core) + wgpu renderer (photrez-render)` | Locked (future) |
| Architecture (MVP runtime) | `Tauri shell + TypeScript DocumentEngine + WebGL2 renderer` â€” see Architecture Migration v2 | Locked (MVP) |
...
| AI handoff standard | Must use `docs/archive/planning/11-implementation-handoff.md` + `docs/archive/planning/12-agent-context-pack.md` for work instructions | Locked |
...
| Full UI shell mockup baseline | Full visual desktop shell reference at `docs/reference/ui-full-editor-mockup.html` | Locked |
...
| Accent color identity | **Photon Amber** (`#E15A17` / `#F97316` / `#C2410C`) â€” migrated from Studio Indigo (`#5C6AEA`). Rationale: warm independent visual identity, suitable for hardware-tooling feel | Locked |
| Ergonomic layout scaling | Menubar `36px` (from 32), Toolbar `42px` (from 38), Status `28px` (from 26), Tool Rail `56px` (from 48), Tool buttons `36Ã—36` (from 32Ã—32), Lucide icons `20Ã—20` (from 18Ã—18). Rationale: click targets too small at 1080p, accessibility | Locked |
| Scrollbar style | Ultra-slim `4px` WebKit scrollbar with `var(--color-text-muted)` thumb. Rationale: reduces visual noise, feels more native | Locked |
...
| Ponytail refactor doctrine | Refactor-from-scratch planning is governed by `docs/ponytail-refactor-doctrine/`: delete/simplify first, prefer native and existing helpers, avoid framework-like abstractions, and introduce only the smallest modulee that removes real current complexity. | Locked 2026-06-17 |
