# AI_HISTORY_ARCHIVE.md â€” Photrez
...
- Setelah `setPointerCapture`, browser mengirim semua pointer events ke capture target (`<svg>`), bukan ke `<g>` — jadi `pointermove`/`pointerup` tidak pernah sampai ke handler.
...
**Root Cause — Free Mode Bukan Free:**
...
**Files Changed:**
- `apps/desktop/src/components/editor/CropOverlay.tsx` — SolidJS event attributes, capture ke `<g>`, transparent hit rects, `onLostPointerCapture`, `CropResizeOptions` call
- `apps/desktop/src/components/editor/CanvasViewport.tsx` — fixed `ensureCropRect`, removed `cropDragState`, `applyCrop` in keyboard Enter handler
- `apps/desktop/src/components/editor/OptionBar.tsx` — Apply button uses `engine.applyCrop()` with `deleteCroppedPixels` + `targetSize`
- `apps/desktop/src/viewport/cropGeometry.ts` — `CropResizeOptions` interface, `applyCropResizeHandle` with constraint modes, modular helper fns
- `apps/desktop/src/engine/document.ts` — `applyCrop()` method with `deleteCroppedPixels` + `targetSize` options
- `apps/desktop/src/components/editor/__tests__/CropOverlay.test.tsx` — NEW: 5 pointer capture regression tests
- `apps/desktop/src/__tests__/crop-geometry.test.ts` — refactored for new options API, +8 test cases
- `apps/desktop/src/engine/__tests__/document.test.ts` — +4 applyCrop tests

**Verifikasi:**
- ✅ `npx vitest run`: 198 PASS (18 files, +16)
...
7. Rotate 90° buttons (↺/↻) in crop option bar.

**Task 1 — Draw New Crop Box:**
...
**Task 2 — Arrow Key Nudge:**
- Already implemented in `CanvasViewport.tsx` at lines 509–541: arrow keys move crop by 1px (or 10px with Shift).
...
- Added conversion utilities in `OptionBar.tsx` at module level:
  - `PPI = 96` (web standard)
  - `toUnit(px: number, unit: CropSizeUnit): number` — converts pixels to the target unit.
  - `fromUnit(value: number, unit: CropSizeUnit): number` — converts target unit back to pixels.
...
- `1 in = 96 px`, `1 cm ≈ 37.795 px`, `1 mm ≈ 3.7795 px`.

**Task 6 — Composition Guides:**
- All five guide modes are already implemented in `CropOverlay.tsx`:
  - `none`: no guidelines.
  - `thirds`: 2×2 grid lines dividing into 9 sections (rule of thirds).
  - `grid`: configurable N×N grid (default 5×5).
...
**Task 7 — Rotate 90° Buttons:**
- Added ↺ (rotate -90°, CCW) and ↻ (rotate +90°, CW) buttons next to the angle field in `OptionBar.tsx`.
- Each button adjusts the `cropRotation` signal by ±90°.
- Styled as small icon buttons matching the existing input field sizes.

**Files Changed:**
- `apps/desktop/src/components/editor/EditorContext.tsx` — `cropUndoStack`, `cropRedoStack`, `commitCropState`, `undoLastCrop`, `redoCrop`, `canCropUndo`, `canCropRedo`, `cropSizeUnit` signals
- `apps/desktop/src/components/editor/CropOverlay.tsx` — `commitCropState` in `clearDrag`, pre-drag rect storage
- `apps/desktop/src/components/editor/AppTitleBar.tsx` — Ctrl+Z/Y interception for crop mode in `handleUndo`/`handleRedo`
- `apps/desktop/src/components/editor/OptionBar.tsx` — `CROP_PRESETS` array, preset dropdown with auto-fit, `toUnit`/`fromUnit` conversion helpers, unit dropdown, rotate 90° buttons
- `apps/desktop/src/engine/types.ts` — no changes needed (cropRect is UI-only)
- `apps/desktop/src/engine/document.ts` — no changes needed (cropRect is UI-only)
- `apps/desktop/src/viewport/cropGeometry.ts` — no changes needed

**Verifikasi:**
- ✅ `pnpm --filter photrez-desktop build`: PASS (~8s, 2028 modules)
- ✅ `pnpm --filter photrez-desktop test`: 246/246 PASS (21 files)
- ✅ `cargo test -p photrez-core`: 85/85 PASS
