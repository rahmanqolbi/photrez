...
| FRR-ARCH-002 | Mitigated | MVP runtime ownership is split across TS engine, Rust core docs, and historical diagrams | `ARCHITECTURE.md` now has a concise active MVP runtime diagram and explicitly labels the old large diagram as historical/future-target reference only. | Keep active runtime diagram and registered command table synchronized with code. |
