...
| FRR-MOVE-003 | Mitigated | Canvas pointer capture behavior now uses a helper abstraction | `useCanvasPointerTools.ts` routes safe capture/release through `pointerCapture.ts`, with focused helper tests and CanvasViewport pointer regression coverage. | Keep browser geometry E2E as required for viewport/overlay changes; extend helper usage to overlay-only surfaces as they are refactored. |
...
- FRR-MOVE-003 mitigated: canvas pointer capture/release now uses `trySetPointerCapture()` / `tryReleasePointerCapture()` instead of local ad-hoc try/catch blocks. Focused helper tests cover success, missing targets, and browser capture errors; CanvasViewport pointer tests remain green.
