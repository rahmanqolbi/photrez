...
1. `pnpm.cmd --filter photrez-desktop test`
2. `pnpm.cmd run build`
3. `cargo test -p photrez-core`
4. `cargo test --workspace`
5. `pnpm.cmd --filter photrez-desktop exec playwright test`
6. `pnpm.cmd tauri dev` smoke for app launch, open, edit, export, OS file drop, close
7. Performance measurement for startup, idle RAM, installer size
8. Manual visual pass for Move, Crop, Brush/Eraser, Drag/Drop, Export
