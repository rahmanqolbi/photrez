...
- [ ] Add a failing region-compositing test proving the terminal center becomes visible without receiving a permanent mask.
- [ ] Add a failing duplicate-suppression test for endpoints already occupied by a regular dab.
- [ ] Add a failing production-overlay test proving non-final preview performs a region pass while final preview does not.
- [ ] Run focused tests and confirm failures describe the missing transient behavior.
...
- [ ] Add point-equality and region-scoped tip compositing helpers.
- [ ] Composite the transient tip after the persistent brush or eraser preview pass.
- [ ] Preserve lock-transparency processing after both brush preview passes.
...
- [ ] Run focused mask, overlay, pointer, renderer, and CanvasViewport coverage.
- [ ] Run the full frontend suite and production build.
- [ ] Run Rust core and workspace tests.
- [ ] Run `git diff --check`, audit mask immutability, and record evidence.
