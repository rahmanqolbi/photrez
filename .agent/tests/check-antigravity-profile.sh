...
for skill in "${required_skills[@]}"; do
  ...
  if grep -qE '^name:[[:space:]]*[^[:space:]].*$' "$file"; then
    pass "$skill has name field"
  else
    fail "$skill missing name field"
  fi

  if grep -qE '^description:[[:space:]]*[^[:space:]].*$' "$file"; then
...
for pattern in "${legacy_patterns[@]}"; do
  if grep -qr "$pattern" "$AGENT_DIR/skills"; then
...
for pattern in "${mapping_checks[@]}"; do
  if grep -qE "$pattern" "$AGENT_DIR/AGENTS.md"; then
